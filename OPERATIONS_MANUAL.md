...
Regression test suite. The toolkit currently has **15 test suites with 130+ individual tests**, all of which must pass before any change is committed.
...
```
15 test suites, all green, ~38s wall time:
  test_comment_preserving_ini.py
  test_comment_preserving_multiline.py
  test_gui_save_roundtrip.py
  test_engine_validation_wiring.py
...
Total individual tests across the toolkit: 133+
```
